...
#'
#' @description
#' Extract events detected by BattyBirdNET-Analyzer from original recording files. As labels species names and a time-stamp are used. As guidance for manual inspection of results, a maximum of 10 samples per taxon are randomly selected for validation. Selected segments are highlighted using the keyword 'validate' in the Quality column of the resulting `BattyBirdNET.xlsx` file. Furthermore, the Comments column is populated with a consecutive numbering (1:N) per taxon.
#'
#' @details
#' There are a couple of important prerequisites:
#'
#' (1) Run analyzer.py to detect events, making sure that --i and --o specify the same location.
#' (2) Reshape BattyBirdNET results using function \code{\link{BirdNET}}
#'
#' @param path path. Relative path will be converted to absolute path.
#' @param output path to write wav files
#' @param taxon optional. select target taxa
...
#' @export
#'
BattyBirdNET_extract <- function(path = NULL,
                            taxon = NULL,
                            score = NULL,
                            nmax = NULL,
                            sec = 1,
  ...
  ## 1.) Check for BattyBirdNET.xlsx and load if present ----------------------------
  if (!file.exists(file.path(path, "BattyBirdNET.xlsx"))) stop("BattyBirdNET.xlsx not found")
  xlsx <- readxl::read_xlsx(file.path(path, "BattyBirdNET.xlsx"))

  ## check if BattyBirdNET.xlsx was already formatted with BattyBirdNET_extract
  ## Attempt by all files unique and containing extracted in file name
  if (!any(duplicated(xlsx$File)) & stringr::str_detect(xlsx$File[1], "extracted")) {
    stop("BattyBirdNET.xlsx already formatted using BattyBirdNET_extract!. Run again with BattyBirdNET() will override existing data")
  }
  ...
  out <- pbapply::pbsapply(1:nrow(xlsx), function(r) {
    ...
    ## find wav file -----------------------------------------------------------
    wav <- stringr::str_replace( xlsx[[r, "File"]], "bat.results.txt", "WAV")
    format <- ".WAV"
  ...
  if (isTRUE(hyperlink) & isFALSE(spectro)) {
    ...
    ## open workbook as it is ...
    wb <- openxlsx::loadWorkbook(file.path(path, "BattyBirdNET.xlsx"))
    ...
    openxlsx::writeData(wb = wb,
                        ...
                        colNames = FALSE,
                        startRow = 2)
    openxlsx::saveWorkbook(wb, file.path(path, "BattyBirdNET.xlsx"), overwrite = TRUE)
  } else if (isTRUE(hyperlink) & isTRUE(spectro))  {
