...
#' (2) Copy extracted events for future references to local database, distinguishing between true and false positives
...
#' @param trash logical. Delete raw files? defaults to FALSE
...
BirdNET_archive <- function(
  ...
  Verification <- T1 <- Taxon <- Hour <- n <- . <- child <- NA
  ...
  if (nrow(df) == 0) stop("No verified detections in ", BirdNET_results, "!")
  ...
  folder.df$file.new[folder.df$file %in% folder.df.true$file] <- folder.df.true$to_path
  ...
  if (isTRUE(keep.false)) {
    ...
    folder.df$file.new[folder.df$file %in% folder.df.false$file] <- folder.df.false$to_path
