...
```{r, eval=FALSE, comment="Install"}
devtools::install_github("mottensmann/NocMigR2")
```

Load the package once installed:

```{r, warning=FALSE, results='hide', comment="load NocMigR2"}
library(NocMigR2)
```
...
```{r, comment="rename", results='markup'}
## Example: 
...
```{r, comment="dusk2dawn", results='markup'}
## Example
## -------
dusk2dawn(
  date = Sys.Date(), ## Date
...
```{r, comment="NocMig_meta", results='markup'}
## usage
## -------
...
## -----------------------------------------------------------------------------
cd ../BirdNET-Analyzer
python3 analyze.py --i ../NocMigR2/test_folder --o ../NocMigR2/test_folder --min_conf 0.7 --rtype 'audacity' --threads 1 --locale 'de'
```
...
```{r, eval=T, comment="split_wave", results='markup'}
## split in segments
